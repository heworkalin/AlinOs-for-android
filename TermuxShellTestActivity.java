...
public class TermuxShellTestActivity extends AppCompatActivity implements ServiceConnection {
    ...
    // ---- 屏幕轮询（替代 PtySession 的 onTextChanged，供 TermuxService 创建的会话使用） ----
    ...
    static class PtySession {
        ...
        /** 包装 TermuxService 创建的已有 TerminalSession（不创建新 PTY） */
        PtySession(String id, TerminalSession existingSession) {
            this.id = id;
            this.session = existingSession;
            // TermuxService.createTermuxSession() 不会调用 updateSize()，需手动初始化 emulator
            this.session.updateSize(80, 24);
            this.emulator = session.getEmulator();
        }
        ...
        /** 直接获取当前终端画布结构化文本（不经过回调） */
        String getStyledScreen() {
            return dumpStyledScreen();
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 确保 TermuxShellManager 等核心组件在绑定服务前已初始化
    ...
    // ================================================================
    //  会话管理（通过 TermuxService 创建真实会话）
    // ================================================================

    /** 确保默认会话 (default) 存在并连接到 mCurrentSession */
    private void ensureDefaultSession() {
        if (!mIsTermuxReady || mTermuxService == null) return;

        // 同步查找已有 default 会话
        synchronized (sSessions) {
            PtySession existing = sSessions.get(DEFAULT_SESSION_ID);
            if (existing != null && existing.isAlive()) {
                existing.setScreenCallback(styled -> runOnUiThread(() -> updateStructuredOutput(styled)));
                mCurrentSession = existing;
                refreshSessionInfo();
                startScreenPolling();
                updateStructuredOutput(existing.getStyledScreen());
                return;
            }
        }

        // 不存在则在 UI 线程创建（TermuxService 操作必须位于 UI 线程，
        // 否则 TermuxSessionsListViewController 的 ListView adapter 会因并发修改崩溃）
        try {
            ...
            TermuxSession termuxSession = mTermuxService.createTermuxSession(
                bashPath, null, null, workingDir, false, DEFAULT_SESSION_ID);

            if (termuxSession == null) return;

            termuxSession.getExecutionCommand().isPermanent = true;
            mTermuxService.addPermanentSessionHandle(termuxSession.getTerminalSession().mHandle);

            TerminalSession terminalSession = termuxSession.getTerminalSession();
            PtySession session = new PtySession(DEFAULT_SESSION_ID, terminalSession);
            session.setScreenCallback(styled -> runOnUiThread(() -> updateStructuredOutput(styled)));
            synchronized (sSessions) { sSessions.put(DEFAULT_SESSION_ID, session); }
            mCurrentSession = session;

            refreshSessionInfo();
            startScreenPolling();
            updateStructuredOutput(session.getStyledScreen());
    ...
    private void createNewSession() {
        ...
        try {
            ...
            // 通过 TermuxService 创建真实会话，使其出现在抽屉和通知中
            TermuxSession termuxSession = mTermuxService.createTermuxSession(
                bashPath, null, null, workingDir, false, sid);

            if (termuxSession == null) {
                appendResult("创建会话失败：TermuxService.createTermuxSession() 返回 null");
                return;
            }
            ...
            // 用 TermuxService 创建的 TerminalSession 包装 PtySession
            TerminalSession terminalSession = termuxSession.getTerminalSession();
            PtySession session = new PtySession(sid, terminalSession);
            session.setScreenCallback(styled -> runOnUiThread(() -> updateStructuredOutput(styled)));
            synchronized (sSessions) { sSessions.put(sid, session); }
            mCurrentSession = session;

            appendResult("会话 " + sid + " 创建成功（已注册到 TermuxService）");
            refreshSessionInfo();
            startScreenPolling();
        } catch (Exception e) {
    ...
    private void connectSession() {
        ...
        mCurrentSession = s;
        s.setScreenCallback(styled -> runOnUiThread(() -> updateStructuredOutput(styled)));
        startScreenPolling();
        updateStructuredOutput(s.getStyledScreen());
    ...
    private void closeCurrentSession() {
        if (mCurrentSession != null) {
            // 从 TermuxService 中移除
            if (mTermuxService != null) {
                mTermuxService.removePermanentSessionByName(mCurrentSession.id);
            }
            ...
            synchronized (sSessions) {
                for (PtySession s : sSessions.values()) {
                    if (s.isAlive()) { hasAlive = true; break; }
                }
            }
            if (!hasAlive) stopScreenPolling();
    ...
    // ================================================================
    //  屏幕轮询（因为 TermuxService 创建的 TerminalSession 已有自己的客户端，
    //  无法通过 PtySession 的 onTextChanged 获取回调，改为定时轮询）
    // ================================================================
    ...
    private void sendKeyToSession(String sequence) {
        ...
        // 立即刷新终端画面（TermuxService 的 TerminalSession 不会触发我们的回调）
        mCurrentSession.refreshScreen();
    }
    ...
    /** 自动创建 PTY 会话（通过 TermuxService），创建后插入文本 */
    private void autoCreateAndSend(String text) {
        ...
        try {
            ...
            setResultText("启动会话失败: " + e.getMessage());
        }
    }
